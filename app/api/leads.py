...
@router.post(
    ...
    description=(
        ...
        "активного маршрута лендинга порождает ``Delivery``. Отправка в "
        "каналы происходит инлайн внутри того же запроса параллельно "
        "через ``asyncio.gather``: ответ возвращается после попытки "
